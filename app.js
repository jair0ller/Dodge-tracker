...
index 325ca044237ea210e1fd419ae5558c9797486d2e..9b53a9d832fe8d40041f5ab0530c10dd30bd8329 100644
--- a/app.js
+++ b/app.js
@@ -1,537 +1,85 @@
 const API_URL =
...
-  }
-  saveState();
+function animateCoin(move) {
+  els.coinBadge.classList.remove("up", "down", "search");
+  void els.coinBadge.offsetWidth;
+  els.coinBadge.classList.add(move === "up" ? "up" : move === "down" ? "down" : "search");
 }
...
+    const coin = data.dogecoin;
+    const price = Number(coin?.usd);
+    const dayChange = Number(coin?.usd_24h_change);
+    const updatedAt = Number(coin?.last_updated_at);
...
+    const instantDelta = previousPrice == null ? NaN : price - previousPrice;
+    const move = previousPrice == null ? "search" : price > previousPrice ? "up" : price < previousPrice ? "down" : "search";
...
-}
+    els.price.textContent = formatPrice(price);
+    els.instantChange.textContent = formatDelta(instantDelta);
+    els.dayChange.textContent = formatPercent(dayChange);
+    els.lastUpdated.textContent = updatedAt
+      ? `Last updated: ${new Date(updatedAt * 1000).toLocaleTimeString([], { hour: "numeric", minute: "2-digit", second: "2-digit" })}`
+      : "Last updated: —";
...
-});
+els.refreshBtn.addEventListener("click", fetchPrice);
